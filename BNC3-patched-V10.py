#!/usr/bin/env python3
"""
Brave New Commune  —  BNC3-patched-V10.py  v018
================================================
CHANGES v013
─────────────────────────────────────────────────────
MODEL:
...
def calculate_semantic_turbulence(raw_text: str, friction_score: float) -> float:
    ...
    entropy = len(set(words)) / (len(words) + 1)
    turbulence = (entropy * 0.7) + (friction_score * 0.3)
...
class BraveNewCommune3:
    ...
    def _write_board(self, record: dict):
        ...
        self.board_jsonl.write_text(json.dumps(record) + "\n", encoding="utf-8")
        self.board_txt.write_text(
            f"[{record['ts']}] {record['agent']}: {record['content']}\n",
            encoding="utf-8",
        )
        
        # V10: Update agent's board entries for anti-repetition
        if record["agent"] in self.states:
    ...
    def _write_colab(self, agent: dict, content: str):
        ...
        self.colab_jsonl.write_text(json.dumps(record) + "\n", encoding="utf-8")
        self.colab_txt.write_text(
            f"[{record['ts']}] {record['agent']}: {record['content']}\n",
            encoding="utf-8",
        )
        self.rag.add_document(
    ...
    def _write_rule(self, agent: dict, content: str):
        ...
        self.rules_jsonl.write_text(json.dumps(record) + "\n", encoding="utf-8")
        self.rules_txt.write_text(
            f"[{record['ts']}] {record['agent']}: {record['content']}\n",
            encoding="utf-8",
        )
    ...
    def _write_diary(self, agent: dict, content: str):
        ...
        diary_jsonl.write_text(json.dumps(jsonl_record) + "\n", encoding="utf-8")
        
        # TXT format (V10: Legacy format for compatibility)
        diary_txt.write_text(
            f"[{self.now_iso()}] {content}\n",
            encoding="utf-8",
        )
