#!/usr/bin/env python3
"""
Brave New Commune  —  bravenewcommune3.py  v011
================================================
CHANGES v011
─────────────────────────────────────────────────────
MODEL:
...
MEMORY — NO CAPS (64GB RAM / 2TB storage / i9):
  • NUM_CTX raised: 4096 → 32768 (saturates gemma4:e4b's full context window)
  ...
  • diary context window: last 10 → last 50 entries shown per context call
  • colab context window: last 8 → last 40 entries shown per context call
  • board context window: last 20 → last 80 posts shown per context call
  • rules: all shown (no cap change — usually small)
  • RAG retrieve: k=3, max_chars=1000 → k=10, max_chars=6000
  • RAG query source window: board[-3] → board[-10]
  • RAG query raw chars: 400 → 2000
  • Library chunk size: 800 → 3000 chars per chunk
  • Library context shown: 800 → 6000 chars per call
  • Axiom audit board context: last 12 → last 30 posts
  • Axiom audit diary context: last 4 → last 15 entries

TOKEN LIMITS RAISED (uncapped for i9 + big RAM):
  • Board posts:    1200 → 2000   (room for full reasoning)
  • Diaries:        700 → 1800    (rich reflection)
  • Colab notes:    600 → 1500    (detailed collaboration)
  • Admin replies:  2000 → 3000   (thorough responses)
  • Proposal write: 1800 → 4000   (complete artifacts)
  • Axiom audit:    2800 → 4000   (deep belief work)
  • Proposal extract: 1300 → 2000
  • Rules session:  380 → 800
  • Memory kernel:  300 → 600     (richer compression summaries)
  • Retry fallback: 275 → 500

PENDING PROPOSALS in context: last 5 → last 20
"""
...
class LibraryReader:
    # 3000 chars per chunk — far more context per library snippet
    CHUNK_SIZE = 3000
...
class SimpleRAGMemory:
    ...
    def retrieve(self, query: str, agent: str = "", k: int = 10, max_chars: int = 6000) -> str:
        ...
        for idx, doc in enumerate(self.docs):
            ...
            d_norm = math.sqrt(sum((f * self._idf(t)) ** 2 for f in doc["counts"].values())) or 1.0
...
class OllamaClient:
    # 32768 — gemma4:e4b's full context window.
...
class BraveNewCommune3:
    ...
    def _context(
        ...
        # Diary — last 50 entries (was 10)
        if st.diary_entries:
            recent = st.diary_entries[-50:]
            parts.append("Your recent diary + memory kernels:\n" + "\n\n".join(recent))

        # Colab — last 40 entries (was 8)
        if st.colab_entries:
            recent = st.colab_entries[-40:]
        ...
        # Board — last 80 posts (was 20)
        if self.board_records:
            recent = self.board_records[-80:]
            parts.append(
        ...
        if pending:
            lines = [f"  [{p['agent']}] {p['title']}: {p['description'][:120]}" for p in pending[-20:]]
        ...
        if include_library and not self.library.is_empty:
            lib_ctx = self.library.get_context(max_chars=6000)
            if lib_ctx:
        ...
        # RAG — k=10, max_chars=6000 (was k=3, 1000)
        if use_rag:
            focus_text = ""
            try:
            ...
            rag_query = " ".join(filter(None, [
                ...
                focus_text,
                " ".join(r["content"] for r in self.board_records[-10:]),
            ]))[:2000]
            rag_ctx = self.rag.retrieve(rag_query, agent=agent["name"], k=10, max_chars=6000)
            if rag_ctx:
    ...
    def _evolve_axioms(self, agent: dict):
        ...
        # Broader audit context — last 30 board posts, last 15 diary entries
        recent_board = "\n".join(f"{r['agent']}: {r['content']}" for r in self.board_records[-30:])
        recent_diary = "\n\n".join(st.diary_entries[-15:])
    ...
    def run(self):
        ...
        print(
            ...
            f"context diary[-50] colab[-40] board[-80])"
        )
